...
def api_forecast(name):
    ...
    # Structure data for React
    data = {
        "name": name,
        "currentPrice": current_price,
        "maxPrice": max_crop, # [Month, Price]
        "minPrice": min_crop, # [Month, Price]
        "forecast": [
            {"month": val[0], "price": val[1], "change": val[2]} 
            for val in forecast_crop_values
        ],
        "previous": [
            {"month": val[0], "price": val[1]} 
            for val in prev_crop_values
        ],
        "cropInfo": {
            "image": crop_data[0],
            "primeLocation": crop_data[1],
            "type": crop_data[2],
            "export": crop_data[3]
        }
    }
...
def api_trends():
    return jsonify({
        "topWinners": TopFiveWinners(),
        "topLosers": TopFiveLosers(),
        "sixMonthForecast": SixMonthsForecast()
    })
...
def SixMonthsForecastHelper(name):
    ...
    for i in commodity_list:
        crop_name = i.getCropName().split('/')[-1].lower()
        if name == crop_name:
            commodity = i
            break
    month_with_year = []
